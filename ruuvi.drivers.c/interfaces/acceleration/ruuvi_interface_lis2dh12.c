...
{
  lis2dh12_fm_t mode;
  lis2dh12_fifo_watermark_set(&(dev.ctx), 9); //Set watermark level to 9-> read 9+1 samples
...
ruuvi_driver_status_t ruuvi_interface_lis2dh12_fifo_read(size_t* num_elements, axis3bit16_t* raw_array)//ruuvi_interface_acceleration_data_t* data)//V
{
//  if(NULL == num_elements || NULL == data) { return RUUVI_DRIVER_ERROR_NULL; }
    if(NULL == num_elements || NULL == raw_array) { return RUUVI_DRIVER_ERROR_NULL; }
  ...
  // Read all elements
  axis3bit16_t raw_acceleration;//V
  float acceleration[3];
  for(size_t ii = 0; ii < 10; ii++)
  {
    //err_code |= lis2dh12_acceleration_raw_get(&(dev.ctx), raw_acceleration.u8bit);//V
    err_code |= lis2dh12_acceleration_raw_get(&(dev.ctx), raw_acceleration.u8bit);
    raw_array[ii].i16bit[0] = raw_acceleration.i16bit[0];
    raw_array[ii].i16bit[1] = raw_acceleration.i16bit[1];
    raw_array[ii].i16bit[2] = raw_acceleration.i16bit[2];
    //raw_array[ii].u8bit[0] = raw_acceleration.u8bit[0];
    //raw_array[ii].u8bit[1] = raw_acceleration.u8bit[1];
    //raw_array[ii].u8bit[2] = raw_acceleration.u8bit[2];
    //raw_array[ii].u8bit[3] = raw_acceleration.u8bit[3];
    //raw_array[ii].u8bit[4] = raw_acceleration.u8bit[4];
    //raw_array[ii].u8bit[5] = raw_acceleration.u8bit[5];
...
ruuvi_driver_status_t ruuvi_interface_lis2dh12_fifo_interrupt_use(const bool enable)
{
  ...
  if(true == enable)
  {
    ...
    err_code |= lis2dh12_fifo_watermark_set(&(dev.ctx), 9);
...
ruuvi_driver_status_t cowbhave_fifo_data_level_get(uint8_t *elements){
  ruuvi_driver_status_t err_code = RUUVI_DRIVER_SUCCESS;
  err_code |= lis2dh12_fifo_data_level_get(&(dev.ctx), elements);
  return err_code;
}

ruuvi_driver_status_t cowbhave_acceleration_raw_get(uint8_t* buff)
{ 
  ruuvi_driver_status_t err_code = RUUVI_DRIVER_SUCCESS;
  err_code |= lis2dh12_acceleration_raw_get(&(dev.ctx), buff);
  return err_code;
}
